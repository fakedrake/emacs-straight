...
;;
;; Copyright (C) 2013 Christos Perivolaropoulos
;;
...
(defun LaTeX-edit-Lua-code-start ()
  ...
  (if (member (LaTeX-current-environment) LaTeX-Lua-environments)
      ...
      (let* ((lua-parent-buffer (current-buffer))
        ...
        (setq-local LaTeX-edit-Lua-code-parent-buffer lua-parent-buffer)
        (setq-local LaTeX-edit-Lua-code-parent-buffer-point lua-where-edit-started)
...
(defun LaTeX-edit-Lua--trim (str)
  (replace-regexp-in-string "^\n+" ""
                            (replace-regexp-in-string "\n+$"  "" str)))

(define-minor-mode auctex-lua-mode
  "Like lua but can exit"
 ;; The initial value.
 :init-value nil
 ;; The indicator for the mode line.
 :lighter " Lua-Tex-minor"
 ;; The minor mode bindings.
 :keymap
 (let ((map (make-sparse-keymap)))
            (define-key map (kbd "C-c C-c") 'LaTeX-edit-Lua-code-finish)
            map))


(provide 'fd-auctex-lua)
